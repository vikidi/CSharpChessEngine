...
namespace ChessEngine
{
  ...
  {
    private const string NAME_VERSION = "ChessEngine v0.0.1";
    private const string AUTHOR = "Ville Saarinen";
    ...
    private static async Task Main()
    {
      ...
      while (true /*_engine.Running*/)
      { 
        string? input = await Task.Run(Console.ReadLine);
        if (input != null) ParseUciCommand(input);
      }
    }
    ...
    private static void ParseUciCommand(string input)
    {
      ...
      switch (tokens[0])
      {
        case "uci":
          ...
          Console.WriteLine($"option name Hash type spin default {"random"/*Transpositions.DEFAULT_SIZE_MB*/} min 1 max 2047"); //consider gcAllowVeryLargeObjects if larger TT is needed
    ...
    {
      if (int.TryParse(Token(tokens, name), out value)) return true;
    ...
    {
      if (long.TryParse(Token(tokens, name), out value)) return true;
